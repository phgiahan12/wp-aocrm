...
class Acf_ct_database_manager
{
	public static function get_create_table_sql_query($acf_post_id, $table_name){
		...
		foreach ($fields as $field) {
			...
			$single_column_query = "`{$field['name']}` {$column_types} NOT NULL,"; //name tinytext NOT NULL,
			if($columnQuery === ""){
	...
	public static function get_acf_fields_change_list($acf_post_id){
		...
		$new_fields = array_diff_assoc($new_acf_fields, $old_acf_fields);
		$deleted_fields = array_diff_assoc($old_acf_fields, $new_acf_fields);
